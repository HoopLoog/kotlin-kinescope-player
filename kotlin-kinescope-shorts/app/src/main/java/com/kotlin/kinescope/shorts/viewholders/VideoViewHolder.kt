...
class VideoViewHolder(
    ...
    init {
        ...
        binding.btnOffline.setOnClickListener {
            videoData?.let { data ->
                ...
                val existingDownload = io.kinescope.sdk.shorts.download.VideoDownloadManager.getDownloadById(contentId)
            ...
            override fun run() {
                videoData?.let { data ->
                    ...
                    if (currentContentId == contentId) {
                        val download = io.kinescope.sdk.shorts.download.VideoDownloadManager.getDownloadById(contentId)
                        if (download != null && 
    ...
    fun startOfflineDownload(videoData: VideoData, keySetId: ByteArray) {
        ...
        try {
            ...
            io.kinescope.sdk.shorts.download.VideoDownloadService.startDownload(context, downloadRequest)
    ...
    private fun showOfflineVideosPopup() {
        ...
        val allDownloads = io.kinescope.sdk.shorts.download.VideoDownloadManager.getAllDownloadsWithActiveFirst(context).toMutableList()
        val adapter = io.kinescope.sdk.shorts.view.DownloadPopupAdapter(context, allDownloads)
        ...
        adapter.setOnItemClicked { download ->
            if (download.state == Download.STATE_COMPLETED) {
                ...
                val (percent, bytesDownloaded) = io.kinescope.sdk.shorts.download.VideoDownloadManager.getDownloadProgress(download)
        ...
        adapter.setOnDeleteClicked { download ->
            io.kinescope.sdk.shorts.download.VideoDownloadManager.removeDownload(context, download.request.id)
        ...
        val downloadListener = object : DownloadManager.Listener {
            ...
            override fun onDownloadChanged(
                ...
                handler.post {
                    val updatedDownloads = io.kinescope.sdk.shorts.download.VideoDownloadManager.getAllDownloadsWithActiveFirst(context)
        ...
        io.kinescope.sdk.shorts.download.VideoDownloadManager.addDownloadListener(context, downloadListener)
        ...
        popupWindow?.setOnDismissListener {
            ...
            io.kinescope.sdk.shorts.download.VideoDownloadManager.removeDownloadListener(downloadListener)
            popupWindow = null
        }
