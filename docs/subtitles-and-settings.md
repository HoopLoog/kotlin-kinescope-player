...
A top-docked panel clears the system safe area at the top (status bar, display cutout) on its own; `captionsSearchTopInset` (px) adds the host's own header drawn over the band. Negative values clamp to 0. While the panel is docked to the top, scrubbing the seek bar does not raise the scrub hint header or lift the control overlay over the panel.
